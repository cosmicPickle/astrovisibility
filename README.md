...
- **Framing:** Under **Optics → Orientation**, choose **AltAz**, **EQ**, or **AltAz + field rotator** and set the angle. Settings save with each optics setup. AltAz measures the angle from local up; EQ and active field derotation measure it from celestial north. With optics selected, visibility requires the entire frame to clear the mask, including any obstruction inside it. Without optics, visibility uses the target center. A profile without a mask remains unassessed.
