...
- **Framing:** Open **Optics** and choose **AltAz**, **EQ**, or **AltAz+FieldCorr** in the segmented row beneath **Orientation**. FieldCorr means active field derotation. Open **Orientation** to set the angle. Settings save with each optics setup. AltAz measures the angle from local up; EQ and active field derotation measure it from celestial north. With optics selected, visibility requires the entire frame to clear the mask, including any obstruction inside it. Without optics, visibility uses the target center. A profile without a mask remains unassessed.
...
- **Window correction (optional):** After saving the mask, define the window's four inside corners and approximate clear width. Later use **Define window** / **Redefine window** in the profile menu. The editor can reset corners or remove the window without changing your original mask. Add the lens's sideways offset in optics setup (blank means zero). Capture near the mount's turning axis at imaging-lens height. This approximation preserves obstacles inside the opening and assumes newly revealed background beyond its old edge is clear within captured coverage. A yellow outline shows the modeled opening; the photo retains its original viewpoint.
