...
- **View All Targets:** Search and filter the catalogue. Expand **Advanced** to limit minor-axis size in sensor pixels (with selected optics) or total usable visibility in minutes. Size limits retain the global 60 px minimum. Choose **Longest Visible** or **Biggest** ordering; these settings are shared with Sky View options. Targets show every visible interval, and selecting one returns to the positioned target in Sky View.
